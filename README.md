...
### User experience
The web API, **TechTalkService**, maintains an in-memory collection of to-do items per authenticated user. The authenticated user carries the same to-do list across multiple instances of the web app in this sample as well as native clients like the [.NET native (WPF) client](https://github.com/Azure-Samples/active-directory-dotnet-native-aspnetcore).

The ASP.NET Core 2.0 web app, **TechTalkWebApp** enables a user to:
- Sign in. The first time a user signs in, a consent screen prompts the user for permission to access **TechTalkService** and obtain user profile information from Azure Active Directory. Since this is a web app, hosted in a browser, it can be that the user gets immediately signed-in benefiting from Single Sign On with other web applications. 
- Select **TechTalk List** in the app's navigation bar. The user may then:
...
#### Register the service app (TechTalkService-aspnetcore)

1. Navigate to the Microsoft identity platform for developers [App registrations](https://go.microsoft.com/fwlink/?linkid=2083908) page.
1. Select **New registration**.
1. When the **Register an application page** appears, enter your application's registration information:
   - In the **Name** section, enter a meaningful application name that will be displayed to users of the app, for example `TechTalkService-aspnetcore`.
...
1. Select the **Expose an API** section, and:
   ...
   - Enter the following parameters
     ...
     - in **Admin consent display name** type `Access TechTalkService-aspnetcore as a user`
     - in **Admin consent description** type `Accesses the TechTalkService-aspnetcore Web API as a user`
     - in **User consent display name** type `Access TechTalkService-aspnetcore as a user`
     - in **User consent description** type `Accesses the TechTalkService-aspnetcore Web API as a user`
     - Keep **State** as `Enabled`
     - Select **Add scope**

#### Register the client app (TechTalkWebApp-aspnetcore)

1. Navigate to the Microsoft identity platform for developers [App registrations](https://go.microsoft.com/fwlink/?linkid=2083908) page.
1. Select **New registration**.
1. When the **Register an application page** appears, enter your application's registration information:
   - In the **Name** section, enter a meaningful application name that will be displayed to users of the app, for example `TechTalkWebApp-aspnetcore`.
   - Change **Supported account types** to **Accounts in any organizational directory**.
...
1. Select the **API permissions** section
   ...
   - In the list of APIs, select the API `TechTalkService-aspnetcore`.
   - In the **Delegated permissions** section, ensure that the right permissions are checked: **user_impersonation**.
   - Select the **Add permissions** button.

#### Configure authorized client applications for service (TechTalkService-aspnetcore)

For the middle tier web API (`TechTalkService-aspnetcore`) to be able to call the downstream web APIs, the user must grant the middle tier permission to do so in the form of consent.
...
1. In the [Azure portal](https://portal.azure.com), navigate to your `TechTalkService-aspnetcore` app registration, and in the **Expose an API** section, click on **Add a client application**.
   Input the client ID of the client application (`TechTalkWebApp-aspnetcore`) and check **user_impersonation** for authorized scopes.
...
1. Open the `TechTalkService\appsettings.json` file
1. Find the app key `Domain` and replace the existing value with your Azure AD tenant name.
1. Find the app key `TenantId` and replace the existing value with your Azure AD tenant ID.
1. Find the app key `ClientId` and replace the existing value with the application ID (clientId) of the `TechTalkService-aspnetcore` application copied from the Azure portal.

#### Configure the client project

> Note: if you used the setup scripts, the changes below will have been applied for you

1. Open the `TechTalkWebApp\appsettings.json` file
1. Find the app key `Domain` and replace the existing value with your Azure AD tenant name.
1. Find the app key `TenantId` and replace the existing value with your Azure AD tenant ID.
1. Find the app key `ClientId` and replace the existing value with the application ID (clientId) of the `TechTalkWebApp-aspnetcore` application copied from the Azure portal.
1. Find the app key `ClientSecret` and replace the existing value with the key you saved during the creation of the `TechTalkWebApp-aspnetcore` app, in the Azure portal.
1. Find the app key `TechTalkResourceId` and replace the existing value with `api://{TechTalkService_ClientId}`.

### Step 4: Run the sample

In the solution properties, set both projects as startup projects. Set **TechTalkService** to run first. Clean the solution, rebuild it, and then run it.  

On startup, the web API displays an empty web page. This is expected behavior.

Explore the sample by signing in into the web app, clicking on "TechTalk List", signing again if needed, adding items to the To Do list, signing-out, and starting again.  Since the authenticated session is stored in a cookie, the application doesn't require logging in again if the previous session was never signed out.

> [!NOTE]
> The To Do list is stored in memory in this sample. Each time the TechTalkService API is stopped, any to-do lists are reset.
...
- Update of the AzureAdOptions class to add a property to compute the `Authority` from the `instance` and the `tenantID`, and adding two other configuration options for `ClientSecret`,  the `resourceId` of TechTalkService (its clientId) and the base address for this service.
- Added a `TechTalkItem` in models to deserialize the Json sent by the TechTalkService
...
- Step 3: 
    ...
    - Navigate to the appsettings.json files for both the TechTalkService web API and TechTalkWebApp web application. Replace the "Instance" property in the Azure AD section with `https://login.microsoftonline.us/`. 
